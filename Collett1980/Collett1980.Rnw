...
<<dmat,include=FALSE>>=
#The D-statistic is the arc lengths between successive observations.  Unfortunatlely that means we need to be able to order our data somehow.  For now, compute n-by-n matrix of arc-lengths
