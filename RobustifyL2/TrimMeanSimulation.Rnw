...
source("C:/Users/Sta36z/Documents/robustSO3/robustFunctions.R")
#sourceCpp("C:/Users/Sta36z/Documents/robustSO3/robustCpp.cpp")
...
Rs<-ruarsCont(n=n,rangle=rangle,kappa1=kappa,p=p1,S=id.SO3,Scont=Scont)
Qs<-as.Q4(Rs)
tMean<-trimMean(Qs,p2,HnFun)

rot.dist(mean(Qs))
rot.dist(trimMean(Qs,p2,HnFun)$Shat)
rot.dist(trimMean(Qs,p2,HnFun,T)$Shat)
rot.dist(median(Qs))

plot(Rs,center=as.SO3(tMean$Shat),show_estimates='all')
#plot(Rs,center=as.SO3(tMean$Shat),show_estimates='all',interactive=T)
...
Rs<-as.Q4(ruarsCont(n=n,rangle=rangle,kappa1=kappa,p=p1,S=id.SO3,Scont=Scont))
wsHn<-1/sqrt(HnFun(Rs))
wsVals<-1/sqrt(HnFun(Rs,full=T))
wsRDist<-1/MeanMove(Rs,meanT='geometric')
wsEDist<-1/MeanMove(Rs)

rot.dist(weighted.mean(Rs,wsHn))
rot.dist(weighted.mean(Rs,wsVals))
rot.dist(weighted.mean(Rs,wsRDist))
rot.dist(weighted.mean(Rs,wsEDist))
rot.dist(mean(Rs))
rot.dist(median(Rs))
rot.dist(trimMean(Qs,p2,HnFun)$Shat)
@
...
kappa<-10
Rs<-ruarsCont(n=n,rangle,kappa1=kappa,p=p1,S=id.SO3,Scont=Scont,space='Q4')
Hn<-sqrt(HnFun(Rs))
...
################
#Empirical IFs versus expected

kap<-1
d<-1
#Projected Mean
Rs<-ruars(50,rcayley,kappa=kap)
dMean<-kap/(kap+2)  #For cayley, d=kapp/(kapp+2)
rs<-mis.angle(Rs)
mm<-MeanMove(Rs,distT='intrinsic') 
mm<-mm/d/max(mm) #scale MeanMove to fit IF scale
curve(sin(x)/d,from=0,to=pi,ylim=c(0,max(mm)),main='Projected Mean',xlab='r',ylab='IF')
points(rs,mm,pch=19)

#Geometric Mean
Rs<-ruars(50,rcayley,kappa=kap)
rs<-mis.angle(Rs)
mmGeom<-MeanMove(Rs,meanT='geometric',distT='intrinsic') 
mmGeom<-mmGeom*pi/2/max(mmGeom) #scale MeanMove to fit IF scale
plot(rs,mmGeom,pch=19,ylim=c(0,pi/2),xlim=c(0,pi),main='Geometric Mean',xlab='r',ylab='IF')
abline(0,1/2)

#Projected Median
Rs<-ruars(50,rcayley,kappa=kap)
dMed<-kap*sqrt(2)*gamma(kap+2)/(3*sqrt(pi)*gamma(kap+2.5))
rs<-mis.angle(Rs)
mmMed<-MedianMove(Rs,distT='intrinsic') 
mmMed<-mmMed/(dMed)/max(mmMed) #scale MeanMove to fit IF scale
curve(sin(x)/(sqrt(1-cos(x))*2*dMed),from=0,to=pi,ylim=c(0,max(mmMed)),
      main='Projected Mean',xlab='r',ylab='IF')
points(rs,mmMed,pch=19)
...
meanErr<-rep(0,11)
meanErr[1]<-round(mis.angle(mean(Rs)),3)
...
for(i in 1:10){
  ...
  meanErr[(i+1)]<-round(mis.angle(hm[[i]]$Shat),3)
