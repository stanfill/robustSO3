...
curve(sin(x)/d,from=0,to=pi,ylim=c(0,max(mm)),main='Projected Mean')
points(rs,mm,pch=19)
...
plot(rs,mmGeom,pch=19,ylim=c(0,pi/2),xlim=c(0,pi),main='Geometric Mean')
abline(0,1/2)
...
mmMed<-mmMed*pi/2/max(mmMed) #scale MeanMove to fit IF scale
curve(sin(x)/sqrt(1-cos(x)),from=0,to=pi,ylim=c(0,max(mmMed)),main='Projected Mean')
