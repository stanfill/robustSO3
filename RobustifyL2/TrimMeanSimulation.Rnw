...
las<-c(expression(-pi),expression(-pi/2),0,expression(pi/2),expression(pi))

qplot(rs,value,data=IFm,geom='line',xlab=expression(r[i]),ylab='Influence',colour=Method,size=I(2))+
  geom_hline(yintercept=0)+geom_vline(xintercept=0)+theme_bw()+coord_fixed()+
  theme(legend.position=c(.16,.875))+
  scale_x_continuous(breaks=c(-pi,-pi/2,0,pi/2,pi),labels=las)+
  scale_y_continuous(breaks=c(-pi,-pi/2,0,pi/2,pi),labels=las)

ggsave("Summary/Figure/IFComparison.pdf",width=5,height=5)
@
