...
ggsave("OutlierDetection/Figure/HnCConcentrated.pdf",height=5,width=6.5)
...
qplot(Hn2,C2,ylab="C",xlab=expression(H[n]),size=I(3),colour=rs2)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCDiffuse.pdf",height=5,width=6.5)
@
...
qplot(Hn,C,xlab=expression(H[n]),size=I(3),colour=rs)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCConcentratedGeometric.pdf",height=5,width=6.5)
...
qplot(Hn2,C2,ylab="C",xlab=expression(H[n]),size=I(3),colour=rs2)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCDiffuseGeometric.pdf",height=5,width=6.5)
@
...
Rs<-ruars(20,rcayley,kappa=50)
shat<-mean(Rs)
Rsi<-matrix(matrix(genR(pi),3,3)%*%shat,1,9) #rotation the estimate mean through pi radians
...
plot(RsNew,center=mean(RsNew),show_estimates='all')
