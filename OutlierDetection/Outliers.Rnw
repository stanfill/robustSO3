...
The following is according to Chapter 12 of \cite{mardia2009} because I cannot get a physical copy of either article.   \cite{best1986} use the relationship between the Watson distribution on the sphere and the exponential distribution to test a discordant point as an outlier.  This is an application of the ideas of FLW to a different distribution and space. \cite{bagchi1990} use Bayesian methods to test for discordant points en bloc, given the number of discordant points is fixed and known.  

In terms of estimators that accommodate outliers,  \cite{wehrly1981} compare the circular mean and median based on traditional influence functions.  Since the influence functions for both estimators are bounded and are therefor B-robust estimators the authors claim the mean is preferred since it is also the MLE. \cite{ko1988} later claim the work of \cite{wehrly1981} (and \cite{watson1983} for that matter) is garbage because using traditional influence curves for bounded parameter spaces is meaningless.  \cite{lenth1981} considers an iteratively re-weighted M-estimator.  The main drawback from his estimator is its reliance on a good estimate of concentration $\kappa$.  \cite{ducharme1987} introduce the {\it normalized spatial median}, which is shown to perform well in the presence of outliers.  \cite{barnett1994} summarize most of this material in Chapter 11.
...
\]
where $\hat{\bm q}_n$ is the projected mean for the full data set and $\hat{\bm q}^{(i)}$ is the projected mean when the $i$th observation has been deleted.  Figures \ref{fig:HnC} and \ref{fig:HnCGeom} illustrate the relationship between these statistics for the projected and geometric means, respectively, in a concentrated data set (left) and a diffuse data set (right).  Both show a strong relationship between $C$ and $H_n$.
...
Hn<-HnFun(Rs)
C<-MeanMove(Rs,distT='intrinsic')
rs<-dist(Rs,mean(Rs),method='intrinsic')

qplot(Hn,C,xlab=expression(H[i]),ylab=expression(C[i]),size=I(3),colour=rs)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCConcentrated.pdf",height=4,width=5.5)

Rs2<-ruars(25,rcayley,kappa=1)
Hn2<-HnFun(Rs2)
C2<-MeanMove(Rs2,distT='intrinsic')
rs2<-dist(Rs2,mean(Rs2),method='intrinsic')
cpi2<-rep("A",nrow(Rs))
cpi2[which(abs(rs2-pi/2)<.05)]<-"B"

qplot(Hn2,C2,ylab=expression(C[i]),xlab=expression(H[i]),size=I(3),colour=rs2)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCDiffuse.pdf",height=4,width=5.5)
@

<<HnCIntrinsic,echo=FALSE,out.width=".5\\textwidth",fig.align='center',fig.show='hold',fig.height=5,fig.width=5,fig.cap="The $H_n$ and $C$ statistics for a sample from the Cayley distribution with $\\kappa=50$ (left) and $\\kappa=1$ (right).",eval=FALSE>>=
mains <- expression(paste(d[R](R[i], hat(S)[R])))
#Rs<-ruars(25,rcayley,kappa=50)
Hn<-HnFun(Rs)
C<-MeanMove(Rs,distT='intrinsic',meanT='geometric')
rs<-dist(Rs,mean(Rs,type='geometric'),method='intrinsic')

qplot(Hn,C,xlab=expression(H[i]),ylab=expression(C[i]),size=I(3),colour=rs)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCConcentratedGeometric.pdf",height=4,width=5.5)

#Rs2<-ruars(25,rcayley,kappa=1)
Hn2<-HnFun(Rs2)
C2<-MeanMove(Rs2,meanT='geometric',distT='intrinsic')
rs2<-dist(Rs2,mean(Rs2,type='geometric'),method='intrinsic')
cpi2<-rep("A",nrow(Rs))
cpi2[which(abs(rs2-pi/2)<.05)]<-"B"

qplot(Hn2,C2,ylab=expression(C[i]),xlab=expression(H[n]),size=I(3),colour=rs2)+theme_bw()+
  scale_colour_gradient(mains)
ggsave("OutlierDetection/Figure/HnCDiffuseGeometric.pdf",height=4,width=5.5)
@
...
#WRONG.  Hn is doing exactly what we want (punishing observations far from the center correctly), the C statistic is what is doing the other thing (punishing observations near pi/2)
...
<<ProjMeanRobst,include=FALSE,eval=FALSE>>=
#I think if you simulate a sample and estimate the mean then add an observation on the complete different pole of the projected mean, the projected mean will not change
Rs<-ruars(20,rcayley,kappa=50)
shat<-mean(Rs)
shatG<-mean(Rs,type='geometric')
Rsi<-matrix(matrix(genR(pi-.0001),3,3)%*%shat,1,9) #rotation the estimate mean through pi radians
RsNew<-as.SO3(rbind(Rs,Rsi))
shatNew<-mean(RsNew)
shatGNew<-mean(RsNew,type='geometric')

shat-shatNew #very near matrix of zeros if I'm right.  I'M RIGHT
dist(shat,as.SO3(Rsi),method='intrinsic') #even though it is pi radians away!
coo<-3
plot(Rs,center=mean(Rs),show_estimates=c('geom.mean','proj.mean'),col=coo)
ggsave("Summary/Figure/Sample1.png",width=4,height=3)
plot(RsNew,center=mean(RsNew),show_estimates=c('geom.mean','proj.mean'),col=coo,label_points=c(rep("",20),"X"))
ggsave("Summary/Figure/Sample2.png",width=4,height=3)
@
