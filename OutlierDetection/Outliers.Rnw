...
C<-MeanMove(Rs)
fit<-lm(Hn~C+I(C^2))
qplot(C,Hn,ylab=expression(H[n]),size=I(3))+stat_smooth(method='loess')+theme_bw()
ggsave("Figure/HnCConcentrated.pdf",height=5,width=5)

Rs2<-ruars(50,rcayley,kappa=1)
Hn2<-HnFun(Rs2)
C2<-MeanMove(Rs2)
fit<-lm(Hn~C+I(C^2))
qplot(C2,Hn2,xlab="C",ylab=expression(H[n]),size=I(3))+theme_bw()
ggsave("Figure/HnCDiffuse.pdf",height=5,width=5)
@
