...
\item \cite{preisig2006} considers robust M-estimators (trimmed, winsorized, Tukey bi-weight, iteratively reweighted least squares) in edge detection for 3D movement tracking.  Read \cite{fletcher2008} and \cite{fletcher2009} for more image related robust papers.  According to \cite{fletcher2008}, \cite{meer1991} and \cite{stewart1999}  are literature reviews of robust methods in computer vision.
